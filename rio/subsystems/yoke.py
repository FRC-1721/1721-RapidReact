...
class Yoke(SubsystemBase):
    ...
    def __init__(self) -> None:
        ...
        self.upperdowner = CANSparkMax(self.yoke_const["shooter"]["upper_downer_id"], CANSparkMaxLowLevel.MotorType.kBrushless)
        
        #this may not be the way to do it, please ckeck later
        self.kicky = CANSparkMax(self.yoke_const["shooter"]["kicky_id"], CANSparkMaxLowLevel.MotorType.kBrushless)
    ...
    def raiseDropSloppy(self, degs, curDegs): #raises or lowers the shooter the inputted amount of degrees
        #degs is the amount to move, curDegs is the degrees off the ground it already is
        if(curDegs+degs<=100 and curDegs+degs>=0): #stops the shooter from going underground or into itself
    ...
    def kickyGoSloppy(self, degs, curDegs): #raises or lowers the kicky the inputted amount of degrees
        #degs is the amount to move, curDegs is the degrees off the ground it already is
        if(curDegs+curDegs<=60 and curDegs+curDegs>=0): #stops the kicky from going underground or into itself
